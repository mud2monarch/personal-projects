...
def get_historical_cex_volume(
    venue: str, start_unix: int, end_unix: int
) -> pl.DataFrame:
    ...
    data = []
    for row in raw:
        new_dict = {"unix_timestamp": row[0] / 1000, "btc_vol": row[1]}
        data.append(new_dict)

    return pl.DataFrame(
        data, {"unix_timestamp": pl.Int64, "btc_vol": pl.Float64}
    ).with_columns(
...
def find_missing_data(df: pl.DataFrame) -> pl.DataFrame:
    ...
    for x in exchanges:
        missing_dates = (
            ...
            .with_columns(
                ...
                ((pl.col("dt").cast(pl.Int64)) * SECONDS_IN_A_DAY).alias(
                    "unix_timestamp"
                )
            )
            .sort("dt")
            .with_columns(
                ...
                (pl.col("unix_timestamp").cast(pl.Float64) / THIRTY_DAYS)
                .floor()
                .alias("group")
            )
            .group_by(
            ...
            .select(["venue", "start_timestamp", "end_timestamp"])
        )
...
def main():
    ...
    )  # theoretically you could pause here to check if you actually want to rip it before doing so
